...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  computed: {
    popClassification: function popClassification(cpop) {
      var i = 6;
      var cat;
  ...
  methods: {
    getData: function getData() {
      ...
      if (!this.selectedCrm) {
        axios.get("/api/criticPops?core=".concat(this.core, "&page=").concat(this.data.current_page)).then(function (response) {
          _this.data = response.data;
        });
      } //Si hay un CRM seleccionado, pero no hay zona seleccionada
      else if (!this.selectedZona) {
          console.log(this.selectedCrm.id);
          axios.get("/api/criticPopsCrm?crm_id=".concat(this.selectedCrm.id, "&core=").concat(this.core, "&page=").concat(this.data.current_page)).then(function (response) {
            _this.data = response.data;
          });
        } // Si hay una zona seleccionada
        else {
            axios.get("/api/criticPopsZona?zona_id=".concat(this.selectedZona.id, "&core=").concat(this.core, "&page=").concat(this.data.current_page)).then(function (response) {
              _this.data = response.data;
            });
          }
    },
              ...
              _vm._l(_vm.data.data, function(item) {
                return _c("tr", { staticClass: "is-size-7" }, [
                  _c(
                    ...
                    [_vm._v(_vm._s(item.cod_planificacion))]
                  ),
                  _vm._v(" "),
                  _c(
                    "td",
                    { staticClass: "has-text-left", class: _vm.primaryText },
                    [_vm._v(_vm._s(item.pop.sites[0].nem_site))]
                  ),
                  _vm._v(" "),
                  _c("td", {
