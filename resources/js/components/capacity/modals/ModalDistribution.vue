...
<script>    
    export default {
        ...
        data() {
            return {
                newTotalCapacity: this.room.current_room_distribution ? this.room.current_room_distribution.total_capacity : 0,
                newUsedCapacity: this.room.current_room_distribution ? this.room.current_room_distribution.used_capacity : 0,
                isEditMode: false,
            }
        },
        ...
        methods: {
            saveChanges() {
                if (!this.isEditMode && 
                    ...
                    // console.log(params)
                    axios.post(`/api/createRoomDistribution/${this.room.id}`, params).then(response => {
