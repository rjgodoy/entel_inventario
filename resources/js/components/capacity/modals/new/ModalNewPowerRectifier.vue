        ...
        <!-- <form action=""> -->
            <div class="modal-card" style="width: auto">
                <header class="modal-card-head">
                    <p class="modal-card-title">Nueva Planta Rectificadora en sala {{ room.name }}</p>
                </header>
    ...
    export default {
        ...
        data() {
            return {
                ...
                // roomDestiny: null,
                planeType: this.selectedPlane && this.selectedPlane.plane_type,
        ...
        computed: {
            canSave() {
                return this.room && this.planeType && this.power_rectifier_type && this.power_rectifier_mode && !this.powerRectifiersWithoutRoom.length && this.capacity > 0
            }
        },
        ...
        methods: {
            ...
            asignRoom(powerRectifier) {
                if(this.room && this.planeType && this.powerRectifiersWithoutRoom.length) {
                    this.$buefy.dialog.confirm({
                        message: `Confirma asignación de la planta rectificadora ${powerRectifier.power_rectifier_type.type} - ${powerRectifier.power_rectifier_type.model} a sala ${ this.room.name }, alimentando al plano ${this.planeType.type}?`,
                        type: 'is-link',
                        onConfirm: () => {
                            axios.put(`/api/powerRectifiers/${powerRectifier.id}?room_id=${this.room.id}&plane_type_id=${this.planeType.id}`)
                            .then(response => {
