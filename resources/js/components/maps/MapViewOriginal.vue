    ...
    export default {
        ...
        data() {
            return {
                ...
                icon: '../img/markers/entel-pin.png',
        ...
        watch: {
            ...
            core(newValue, oldValue) {
                ...
                if (this.selectedCrm == null) {
                    ...
                    this.popsCoreListsCrm()
                } else {
                    this.popsCoreListsZona()
                }
            },
            selectedPops(newValue, oldValue) {
                if (newValue.length != 0) {
                    this.markers = []
                    newValue.forEach(this.popsSelectedLists)
                } else {
                    this.getPops()
                }
            ...
            popsCoreLists() {
                if (this.core == 0) {
                    if (this.pops == null) {
                        ...
                        this.pops.forEach(this.popLocations)
                        this.markersPops = this.markers
                    } else {
                        this.markers = this.markersPops
                    }
                } else {
                    if (this.popsCore == null) {
                        this.popsCore = this.popList
                        this.popsCore.forEach(this.popCoreLocations)
                        this.markersPopsCore = this.markers
                    } else {
                        this.markers = this.markersPopsCore
                    }
                }
            },
            popsCoreListsCrm() {
                if (this.core == 0) {
                    this.popListCrm.forEach(this.popLocations)
                    this.markersPopsCrm = this.markers
                } else {
                    this.popListCrm.forEach(this.popCoreLocations)
                    this.markersPopsCoreCrm = this.markers
                }
            },
            popsCoreListsZona() {
                if (this.core == 0) {
                    this.popListZona.forEach(this.popLocations)
                    this.markersPopsZona = this.markers
                } else {
                    this.popListZona.forEach(this.popCoreLocations)
                    this.markersPopsCoreZona = this.markers
                }
            },
